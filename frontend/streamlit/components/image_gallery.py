...
class ImageGallery:
    ...
    This class provides functionality to display images extracted from web content
    in a gallery format with filtering and interactive features.
    
    Attributes:
        backend_url: Base URL for the backend API service
    """

    def __init__(self, backend_url: str = "http://localhost:8000"):
        """
        Initialize the ImageGallery component.
    ...
    def render_image_gallery(self, content_id: int, title: str = "Extracted Images"):
        ...
        try:
            ...
            logger.error(f"Unexpected error in image gallery: {str(e)}")
            st.error("An unexpected error occurred while loading images")
    ...
    def _fetch_images(self, content_id: int) -> List[Dict[str, Any]]:
        ...
        Args:
            content_id: ID of the content to fetch images for
            
        Returns:
            List of image dictionaries from the API
        """
        try:
            ...
            logger.error(f"Network error fetching images: {str(e)}")
            return []
    ...
    def _filter_images(self, images: List[Dict], show_content: bool, show_decorative: bool, image_type_filter: str) -> List[Dict]:
        ...
        Args:
            images: List of image dictionaries to filter
            show_content: Whether to show content images
            show_decorative: Whether to show decorative images
            image_type_filter: Type filter to apply
    ...
    def _render_image_grid(self, images: List[Dict], content_id: int):
        ...
        Args:
            images: List of image dictionaries to render
            content_id: Content ID for unique keys
        """
    ...
    def _render_single_image(self, image: Dict[str, Any], content_id: int):
        ...
        Args:
            image: Image dictionary containing metadata
            content_id: Content ID for unique keys
        """
        try:
            ...
            with st.container():
                ...
                if image.get('thumbnail_url') and image.get('file_path'):
                    ...
                    try:
                        ...
                        logger.warning(f"Network error loading thumbnail: {str(e)}")
                        st.error("🖼️ Network error loading image")
                    except (OSError, IOError) as e:
                        logger.warning(f"Invalid image format: {str(e)}")
                        st.error("🖼️ Invalid image format")
                    except Exception as e:
                        logger.warning(f"Unexpected error loading thumbnail: {str(e)}")
            ...
            logger.error(f"Missing image data for {image.get('id', 'unknown')}: {str(e)}")
            st.error("Missing image information")
        except Exception as e:
            logger.error(f"Unexpected error rendering image {image.get('id', 'unknown')}: {str(e)}")
    ...
    def _download_image(self, image: Dict[str, Any]):
        ...
        Args:
            image: Image dictionary containing download information
        """
        try:
            if image.get('download_url'):
                download_url = f"{self.backend_url}{image['download_url']}"
