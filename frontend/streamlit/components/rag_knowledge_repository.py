...
try:
    ...
    from backend.src.application.interfaces.llm import LLMRequest
...
class RAGKnowledgeRepository:
    ...
    This class provides a comprehensive knowledge repository that combines:
    - Vector-based similarity search using ChromaDB
    - Intelligent chunking of web content
    - LLM-powered question answering
    - Fallback to rule-based responses
    
    Attributes:
        db_path: Path to the SQLite database
        chroma_path: Path to the ChromaDB vector database
        embedding_model: Sentence transformer model for embeddings
        chroma_client: ChromaDB client instance
        chroma_collection: ChromaDB collection for chunks
    ...
    def __init__(self):
        """
        Initialize the RAG Knowledge Repository.
        
        Sets up the vector database, embedding model, and LLM service.
        Handles graceful fallbacks when dependencies are not available.
        """
        ...
        if EMBEDDINGS_AVAILABLE:
            try:
                ...
                self.embedding_model = None
                self.initialization_messages.append(("error", f"❌ Unexpected error loading embedding model: {e}"))
        else:
        ...
        if CHROMADB_AVAILABLE and self.embedding_model:
            try:
                ...
                self.chroma_client = None
                self.initialization_messages.append(("error", f"❌ Unexpected ChromaDB error: {e}"))
        ...
        if LLM_SERVICE_AVAILABLE:
            try:
                ...
                self.initialization_messages.extend([
                    ("error", f"❌ Unexpected LLM service error: {e}"),
                    ("info", "🔧 **Using enhanced rule-based responses as fallback**")
                ])
        else:
    ...
    def _init_chromadb(self):
        ...
        try:
            ...
            return self._handle_chromadb_error(f"❌ ChromaDB file system error: {e}")
        except (ValueError, TypeError) as e:
            return self._handle_chromadb_error(f"❌ ChromaDB configuration error: {e}")
        except Exception as e:
            return self._handle_chromadb_error(f"❌ Unexpected ChromaDB initialization error: {e}")

    def _handle_chromadb_error(self, error_message: str) -> bool:
        """
        Helper method to handle ChromaDB errors consistently.
        ...
        if question and question.strip():
            ...
            if "website_dropdown_select" in st.session_state and st.session_state.website_dropdown_select:
                ...
                if "All websites" in selected_options:
                    ...
                    selected_titles = [w['title'][:30] + '...' if len(w['title']) > 30 else w['title'] for w in selected_websites]
                    st.info(f"🎯 Querying **{len(selected_websites)} selected website(s)**: {', '.join(selected_titles)}")
            else:
                selected_websites = websites
...
Answer:"""
    ...
    def _extract_meaningful_sentences(self, context: str, max_sentences: int = 4) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if len(sentence) > 20 and not sentence.startswith('**'):  # Skip short and header lines
    ...
    def _generate_no_results_response(self, question: str, websites: List[Dict]) -> str:
        ...
        response = (
            "I couldn't find specific information to answer your question in the current knowledge base.\n\n"
            f"**Available Sources:** {', '.join(website_names)}"
            f"{'...' if len(websites) > 3 else ''}\n\n"
            "**Suggestions:**\n"
    ...
    def _convert_dict_to_analysis_result(self, result_dict):
        """
        Convert a dictionary result to an analysis result object for testing.
        
        Args:
            result_dict: Dictionary containing analysis result data
            
        Returns:
            Analysis result object with proper attributes
        """
        # Create a simple namespace object to hold the data
        from types import SimpleNamespace
        
        # Create the main analysis result object
        analysis_result = SimpleNamespace()
        analysis_result.url = result_dict.get('url', '')
        analysis_result.status = result_dict.get('status', 'completed')
        analysis_result.executive_summary = result_dict.get('executive_summary', '')
        
        # Create scraped content object
        scraped_content_dict = result_dict.get('scraped_content', {})
        scraped_content = SimpleNamespace()
        scraped_content.title = scraped_content_dict.get('title', '')
        scraped_content.main_content = scraped_content_dict.get('main_content', '')
        scraped_content.meta_description = scraped_content_dict.get('meta_description', '')
        
        analysis_result.scraped_content = scraped_content
    ...
    def _display_vector_details(self, relevant_chunks: List[RetrievalResult], top_k_used: int):
        ...
        for idx, result in enumerate(relevant_chunks, 1):
            ...
            with st.container():
                ...
                with col2:
                    title_display = (
                        chunk.website_title[:40] + "..."
                        if len(chunk.website_title) > 40
                        else chunk.website_title
                    )
                ...
                with col3:
                    ...
                    score_color = (
                        "#28a745" if result.similarity_score > 0.7
                        else "#ffc107" if result.similarity_score > 0.4
                        else "#dc3545"
                    )
                    st.markdown(
                        f'<span style="color: {score_color}; font-weight: 500;">'
                        f'Score: {result.similarity_score:.3f}</span>',
                        unsafe_allow_html=True
                    )
                ...
                content_preview = (
                    chunk.content[:150] + "..." if len(chunk.content) > 150
                    else chunk.content
                )
                ...
                with st.expander(f"Vector #{idx} Details", expanded=False):
                    ...
                    if hasattr(chunk, 'embedding') and chunk.embedding is not None:
                        ...
                        if isinstance(chunk.embedding, np.ndarray):
                            ...
                            st.code(
                                f"[{', '.join([f'{v:.4f}' for v in vector_preview])} ...]"
                            )
    ...
    def _display_text_based_chart(self, relevant_chunks: List[RetrievalResult]):
        ...
        for i, result in enumerate(relevant_chunks, 1):
            ...
            st.markdown(
                f'<span style="color: {score_color};">Chunk {i}: {score_bar} '
                f'{result.similarity_score:.3f}</span>',
                unsafe_allow_html=True
            )
    ...
    def _add_test_vector_display(self):
        ...
        for i in range(3):
            chunk = ContentChunk(
                ...
                content=f"This is sample content chunk {i+1} for testing vector "
                        f"display functionality. It contains relevant information that "
                        f"would be retrieved by vector similarity search.",
                chunk_type="paragraph",
                position=i,
